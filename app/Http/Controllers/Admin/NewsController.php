...
class NewsController extends Controller
{
    ...
    /**
     * Ensure file is available in public storage (fix symlink issues on Windows)
     */
    private function ensureFileInPublicStorage($filePath)
    {
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('featured_image') || $request->filled('cropped_image')) {
            ...
            if ($request->filled('cropped_image')) {
                ...
                // Save main image
                Storage::disk('public')->putFileAs('news', $image, $filename);
                $data['featured_image'] = $path;
                ...
                $this->createThumbnail($image->getRealPath(), storage_path('app/public/' . $thumbnailPath), 200, 150);
            }
        }
    ...
    public function destroy(News $news)
    {
        ...
        if ($news->featured_image) {
            Storage::disk('public')->delete($news->featured_image);
            // Delete thumbnail
            $thumbnailPath = 'news/thumbnails/' . basename($news->featured_image);
